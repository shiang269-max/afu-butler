...
import { hasCallName, cleanCallNames } from './call-names';

type ObserverCategory = 'greeting' | 'food' | 'daily' | 'general' | 'none';
type ObserverMode = 'greeting_morning' | 'greeting_night' | 'food' | 'daily' | 'general';
...
export function isObserverMuteCommand(message: string): boolean {
  ...
  const text = cleanCallNames(message).replace(/[，。！？、,.!?～~\s]/g, '');
...
function abortActiveObserver(state: ObserverState, reason: string): void {
  const controller = state.activeAbortController;
  if (!controller || controller.signal.aborted) return;
  console.log(`[Observer] ABORT reason=${reason}`);
  controller.abort(new Error(`Observer task invalidated: ${reason}`));
}
...
export function muteObserver(targetId: string): number {
  ...
  abortActiveObserver(state, 'mute');
...
export function invalidateObserver(targetId: string, eventReceivedAt: number): void {
  const state = getState(targetId);
  state.latestEventReceivedAt = Math.max(state.latestEventReceivedAt || 0, eventReceivedAt);
  abortActiveObserver(state, 'controller-takeover');
  if (state.generalTimer) {
    clearTimeout(state.generalTimer);
    state.generalTimer = undefined;
  }
...
export function observeMessage(options: ObserveOptions): void {
  const traceId = options.diagnosticTraceId || `observer-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`;
  const elapsed = () => options.eventReceivedAt ? Date.now() - options.eventReceivedAt : Date.now();
  const { targetId, userMessage } = options;
  const state = getState(targetId);

  console.log(`[Observer][${traceId}] RECEIVED elapsed=${elapsed()}ms target=${targetId} message=${JSON.stringify(userMessage)}`);

  if (options.eventReceivedAt !== undefined) {
    const previousEvent = state.latestEventReceivedAt;
    if (previousEvent !== undefined && options.eventReceivedAt > previousEvent) {
      abortActiveObserver(state, 'newer-passive-event');
    }
    state.latestEventReceivedAt = Math.max(state.latestEventReceivedAt || 0, options.eventReceivedAt);
  }
  ...
  console.log(`[Observer][${traceId}] CLASSIFIED category=${category} elapsed=${elapsed()}ms`);
...
function getState(targetId: string): ObserverState {
  ...
  if (!state) {
    state = { lastPassiveReplyAt: 0, meaningfulSinceDecision: 0, generalGeneration: 0 };
...
function startPassiveTask(
  ...
  void generateAndReplyPassive({ ...options, mode, fallback, abortSignal: controller.signal }).finally(() => {
    if (state.activeAbortController === controller) state.activeAbortController = undefined;
...
function handleGreeting(options: ObserveOptions, state: ObserverState): void {
  ...
  const type = text.includes('晚安') || /^晚[\s!！。,.，～~]*$/.test(text) ? '晚安' : '早安';
  const now = Date.now();
  if (state.greetingLastSeenAt && now - state.greetingLastSeenAt < GREETING_WAVE_COOLDOWN_MS) return;
  startPassiveTask(options, state, type === '早安' ? 'greeting_morning' : 'greeting_night', type === '早安' ? '早，諸位主子。' : '晚安，諸位主子。');
}

function handleFood(options: ObserveOptions, state: ObserverState): void {
  state.foodLastSeenAt = Date.now();
  if (Date.now() - state.lastPassiveReplyAt < GENERAL_PASSIVE_COOLDOWN_MS) return;
  startPassiveTask(options, state, 'food', '奴才也想吃。');
}

function handleDaily(options: ObserveOptions, state: ObserverState): void {
  const now = Date.now();
  const dailyKey = normalizeDailyKey(options.userMessage);
  if (state.dailyLastKey === dailyKey && state.dailyLastAt && now - state.dailyLastAt < DAILY_DUPLICATE_COOLDOWN_MS) return;
  if (state.observerInFlight) return;
  state.dailyLastKey = dailyKey;
  state.dailyLastAt = now;
  startPassiveTask(options, state, 'daily', '喳，奴才收到。');
}
...
function handleGeneral(options: ObserveOptions, state: ObserverState): void {
  ...
  if (Date.now() - state.lastPassiveReplyAt < GENERAL_PASSIVE_COOLDOWN_MS) return;
  if (state.observerInFlight) return;
  if (state.generalTimer) clearTimeout(state.generalTimer);
  ...
  state.generalTimer = setTimeout(async () => {
    ...
    state.generalTimer = undefined;
    if (Date.now() - state.lastPassiveReplyAt < GENERAL_PASSIVE_COOLDOWN_MS) return;
    if (isObserverMuted(state) || state.observerInFlight) return;
    state.meaningfulSinceDecision = 0;
    startPassiveTask(options, state, 'general', '');
  }, GENERAL_DEBOUNCE_MS);
}

async function generateAndReplyPassive(
  options: ObserveOptions & { mode: ObserverMode; fallback: string; abortSignal: AbortSignal },
): Promise<void> {
  const traceId = options.diagnosticTraceId || `observer-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`;
  const elapsed = () => options.eventReceivedAt ? Date.now() - options.eventReceivedAt : Date.now();
  const { targetId, userMessage, familyMember, getConversationContext, gemini, lineClient, replyToken, replyDeadlineAt, onPassiveReply, mode, fallback, abortSignal } = options;

  console.log(`[Observer][${traceId}] GENERATE_START mode=${mode} elapsed=${elapsed()}ms replyRemaining=${Math.max(0, replyDeadlineAt - Date.now())}ms`);

  try {
    const currentState = getState(targetId);
    if (abortSignal.aborted) return;
    if (!isLatestObserverEvent(currentState, options.eventReceivedAt)) return;
    if (isObserverMuted(currentState) || !isReplyWindowOpen(replyDeadlineAt)) return;
...
不要加任何解釋。不要分析。不要列選項。不要寫前言。不要寫第二句。

被動插話最多 15 個中文字。沒有最低字數。

可以只有：「喳。」「在。」「奴才也想吃。」「這朝會又開始了。」

也可以是一句極短的幽默話。

不要突然寫長篇。不要把家庭對話整理成文章。不要重新分析前面的內容。
...
如果有人突然直接叫你，那屬於正常主動對話，不是這次 Observer 的任務。
    ...
    if (!replyText || replyText === 'NO_REPLY' || replyText.startsWith('NO_REPLY')) {
      if (mode === 'greeting_morning' || mode === 'greeting_night' || mode === 'food' || mode === 'daily') replyText = fallback;
      else return;
    }

    replyText = replyText.replace(/^REPLY:\s*/i, '').replace(/\r?\n/g, '').trim();
    const firstSentence = replyText.split(/[。！？!?]/)[0]?.trim() || replyText;
    replyText = firstSentence.slice(0, MAX_PASSIVE_CHARS);
    if (!replyText || abortSignal.aborted) return;
    if (!isReplyWindowOpen(replyDeadlineAt) || !isLatestObserverEvent(latestState, options.eventReceivedAt) || isObserverMuted(latestState)) return;
    ...
    if (mode === 'greeting_morning' || mode === 'greeting_night') latestState.greetingLastSeenAt = Date.now();
  } catch (error) {
    if (abortSignal.aborted || (error instanceof Error && (error.name === 'AbortError' || /aborted|abort/i.test(error.message)))) {
      console.log(`[Observer][${traceId}] CANCELLED elapsed=${elapsed()}ms`);
      return;
    }
    console.error(`[Observer][${traceId}] ERROR elapsed=${elapsed()}ms`, error);
  }
}

function buildObserverInstruction(mode: ObserverMode): string {
  switch (mode) {
    case 'greeting_morning': return '這是一波「早安」。\n\n請自然接住這個家庭招呼。\n\n這是即時被動插話，不需要刻意等待。\n\n只需要一句很短的回應。';
    case 'greeting_night': return '這是一波「晚安」。\n\n請自然接住這個家庭招呼。\n\n這是即時被動插話，不需要刻意等待。\n\n只需要一句很短的回應。';
    case 'food': return '這是在談吃飯、食物或餐點。\n\n不要搶著回答家人正在問彼此的問題。\n\n只在適合時補一句旁邊人的自然反應。';
    case 'daily': return '這是在談今天的身體狀態、疲累、忙碌、心情或日常。\n\n如果適合，可以自然關心一句。';
    case 'general': return '這是家庭對話中的一般內容。\n\n只有真的適合插話時才回應。\n\n如果沒有必要加入，請輸出 NO_REPLY。';
  }
}

function isLowInformation(text: string): boolean {
  return /^(嗯+|喔+|哦+|噢+|好+|行+|可以|收到|知道了|了解|ok|OK|👍|哈哈+)$/.test(text.replace(/[，。！？、,.!?～~\s]/g, ''));
}

function isGreeting(text: string): boolean {
  return /^(早安|早|晚安|晚)$/.test(text.replace(/[，。！？、,.!?～~\s]/g, ''));
}
