...
function createAiContext(
  ...
  const conversationType =
    event.source.type === 'group' ? 'group' as const : 'private' as const;
...
async function generateVoteOptions(prompt: string): Promise<string[]> {
  try {
    const response = await geminiApiManager.execute(async (gemini) => {
    ...
    await Promise.all(
      events.map(async (event: any) => {
        ...
        if (
          ...
          const replyText = buildStyleResponse('喳，遵旨。奴才先安靜。');
        ...
        if (
          ...
          const replyText = buildStyleResponse('喳，奴才恢復值班。');
        ...
        if (
          hasTrigger &&
          isCallNameHelpIntent(userMessage)
        ) {
        ...
        if (styleSwitchResult.handled) {
          const styleSwitchReply =
            styleSwitchResult.replyText || '角色風格設定已處理。';
        ...
        if (functionHelpResult.handled) {
          const functionHelpReply =
            functionHelpResult.reply || '目前沒有找到這個功能的說明。';
        ...
        if (!shouldInvokeController) {
          ...
          observeMessage({
            ...
            getConversationContext: () => {
              const latestHistory = getMemory(conversationKey);
              return buildConversationPrompt(latestHistory, '');
            },
            gemini,
            lineClient,
        ...
        try {
          const cleanedMessage =
            hasTrigger
              ? cleanTriggerWords(userMessage)
              : userMessage.trim();

          const wantsAll = ALL_TARGET_WORDS.some((word) =>
          ...
          const aiResult = await runAiCore({
            geminiApiManager,
            context: aiContext,
          });
          ...
          await sendAiReply(
            event.replyToken,
            replyText,
            event.source.type === 'group' ? familyTarget : null,
          );
...
async function sendReminderReply(
  ...
  const mentionText = uniqueUserIds
    .map((userId, index) => {
